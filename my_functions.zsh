...
function logout() {
    case $XDG_CURRENT_DESKTOP in
        ...
        *KDE*) qdbus6 org.kde.Shutdown /Shutdown logout  ;;
...
function yte() {
	yt-dlp --extract-audio --audio-format mp3 --default-search ytsearch "$1"
	}
...
function google(){ 
	browser "https://google.com/search?q=$1";
}
	
function search () {
	read -r arr; xdg-open "https://www.ecosia.org/search?q=${arr}";
}
